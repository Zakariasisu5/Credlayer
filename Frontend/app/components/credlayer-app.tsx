...
import { Activity, ArrowRight, BarChart3, BookOpen, Code2, Database, Globe2, KeyRound, LayoutDashboard, Menu, Network, Radar, Search, Settings, ShieldCheck, Sparkles, UserRound, X, Zap } from "lucide-react";
...
const appNav: { href: string; label: string; icon: LucideIcon }[] = [{ href: "/app", label: "Overview", icon: LayoutDashboard }, { href: "/app/dashboard", label: "Dashboard", icon: Radar }, { href: "/app/profile", label: "Profile", icon: UserRound }, { href: "/app/analysis", label: "Analysis", icon: BarChart3 }, { href: "/app/agents", label: "Agents", icon: Sparkles }, { href: "/app/credentials", label: "Credentials", icon: ShieldCheck }, { href: "/app/activity", label: "Activity", icon: Activity }, { href: "/app/settings", label: "Settings", icon: Settings }];
...
function Shell({ children, title, eyebrow, developer = false }: { children: React.ReactNode; title?: string; eyebrow?: string; developer?: boolean }) { 
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const items = developer ? developerNav.map((item) => ({ ...item, icon: Code2 })) : appNav; 
  const pathname = usePathname();
  
  return <><Header /><div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-[1440px]">{title && <AppSidebar developer={developer} />}<main className="min-w-0 flex-1">{title && <div className="sticky top-0 z-10 border-b border-border bg-background/95 backdrop-blur-sm px-5 py-7 lg:px-10"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="flex lg:hidden items-center justify-center w-9 h-9 rounded-lg border border-border bg-card text-muted-foreground hover:text-foreground transition-colors" aria-label="Toggle menu"><Menu className="size-5" /></button><div><p className="font-mono text-[10px] uppercase tracking-[0.2em] text-primary">{eyebrow || "CredLayer"}</p><h1 className="mt-2 text-xl sm:text-2xl font-bold tracking-tight lg:text-3xl">{title}</h1></div></div><WalletControl /></div></div>}{children}</main></div>
  
  {/* Mobile menu overlay */}
  {title && <div className={`fixed inset-0 z-50 lg:hidden transition-all duration-300 ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}><div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setIsMobileMenuOpen(false)} /><div className={`absolute left-0 top-0 bottom-0 w-72 bg-card/95 backdrop-blur-xl border-r border-border shadow-xl transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}><div className="flex items-center justify-between p-4 border-b border-border"><Brand /><button onClick={() => setIsMobileMenuOpen(false)} className="flex items-center justify-center w-9 h-9 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent transition-colors"><X className="size-5" /></button></div><div className="p-4 overflow-y-auto max-h-[calc(100vh-5rem)]"><p className="mb-3 px-2 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{developer ? "Developer console" : "Workspace"}</p><div className="flex flex-col gap-1">{items.map((item) => { const Icon: LucideIcon = item.icon; const active = pathname === item.href; return <Link key={item.href} href={item.href} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition ${active ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-accent hover:text-foreground"}`}><Icon className="size-4" />{item.label}</Link>; })}</div></div></div></div>}
  </>; 
}
...
function DashboardPreview() { return <Shell title="Reputation dashboard" eyebrow="Preview mode"><div className="mx-auto max-w-7xl px-5 py-8 lg:px-10"><div className="mb-6 flex items-center justify-between"><div><p className="text-sm text-muted-foreground">A private view of your verifiable identity.</p></div><Badge tone="amber">Preview · No live data</Badge></div><StyledCard className="mb-5 overflow-hidden p-0"><div className="p-6 pb-0"><p className="font-mono text-[10px] uppercase tracking-widest text-primary">CredLayer verification infrastructure</p><h3 className="mt-2 text-xl font-semibold">Network topology</h3><p className="mt-1 text-sm text-muted-foreground">Live preview of the verification network status</p></div><NetworkVisual dashboard /></StyledCard><div className="grid gap-5 md:grid-cols-3"><Stat label="Reputation score" value="—" note="Connect wallet to view" /><Stat label="Credentials" value="—" note="No credentials issued" /><Stat label="Network activity" value="—" note="Awaiting first transaction" /></div><div className="mt-5 grid gap-5 lg:grid-cols-[1.4fr_1fr]"><StyledCard><div className="flex items-center justify-between border-b border-primary/20 pb-4"><h2 className="font-semibold">Signal overview</h2><BarChart3 className="size-4 text-muted-foreground" /></div><div className="mt-6 space-y-4"><div className="flex items-center justify-between rounded-lg border border-border bg-background/50 p-4"><div><p className="text-sm font-medium">Verified signals</p><p className="mt-1 text-xs text-muted-foreground">Total verification events</p></div><span className="text-2xl font-bold text-primary">—</span></div><div className="flex items-center justify-between rounded-lg border border-border bg-background/50 p-4"><div><p className="text-sm font-medium">Trust connections</p><p className="mt-1 text-xs text-muted-foreground">Linked identities</p></div><span className="text-2xl font-bold text-primary">—</span></div><div className="flex items-center justify-between rounded-lg border border-border bg-background/50 p-4"><div><p className="text-sm font-medium">Attestations</p><p className="mt-1 text-xs text-muted-foreground">Third-party verifications</p></div><span className="text-2xl font-bold text-primary">—</span></div></div><p className="mt-4 text-xs text-muted-foreground italic">Connect a wallet to enable signal analysis</p></StyledCard><StyledCard><div className="border-b border-primary/20 pb-4"><h2 className="font-semibold">Connected identity</h2><p className="mt-1 text-xs text-muted-foreground">Link your wallet to view reputation data</p></div><div className="mt-5 flex flex-col gap-3"><div className="flex items-center justify-between border-b border-border pb-3 text-sm"><span className="text-muted-foreground">Wallet</span><span className="text-amber-400">Not connected</span></div><div className="flex items-center justify-between border-b border-border pb-3 text-sm"><span className="text-muted-foreground">Network</span><span className="text-primary">Solana Devnet</span></div><div className="flex items-center justify-between border-b border-border pb-3 text-sm"><span className="text-muted-foreground">Status</span><span className="flex items-center gap-2"><span className="size-2 rounded-full bg-primary shadow-[0_0_8px_#22d3ee] animate-pulse" />Active</span></div><div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">Privacy mode</span><span className="text-green-400">Enabled</span></div><div className="mt-4"><WalletControl /></div></div></StyledCard></div><div className="mt-5"><StyledCard><div className="flex items-center justify-between border-b border-primary/20 pb-4"><h2 className="font-semibold">Recent activity</h2><Activity className="size-4 text-muted-foreground" /></div><Empty icon={Activity} title="No activity recorded" description="Your verification events and credential issuance will appear here once you connect your wallet." /></StyledCard></div></div></Shell>; }
function AppPage({ sub }: { sub?: string }) { const configs: Record<string, [string, string]> = { dashboard: ["Dashboard", "A private view of your verifiable identity and reputation."], profile: ["Profile", "Your identity, your permissions."], analysis: ["Analysis", "Understand the signals behind your reputation."], agents: ["Agents", "Create trusted workflows for your products."], credentials: ["Credentials", "Portable proof you can share with consent."], activity: ["Activity", "A transparent record of protocol events."], settings: ["Settings", "Manage preferences and connected services."] }; const [title, desc] = configs[sub || ""] || ["Overview", "Your reputation workspace, at a glance."]; return <Shell title={title} eyebrow="App workspace"><div className="mx-auto max-w-7xl px-5 py-8 lg:px-10"><p className="mb-7 text-sm text-muted-foreground">{desc}</p>{sub === "dashboard" ? <><div className="mb-6 flex items-center justify-end"><Badge tone="amber">Preview · No live data</Badge></div><StyledCard className="mb-5 overflow-hidden p-0"><div className="p-6 pb-0"><p className="font-mono text-[10px] uppercase tracking-widest text-primary">CredLayer verification infrastructure</p><h3 className="mt-2 text-xl font-semibold">Network topology</h3><p className="mt-1 text-sm text-muted-foreground">Live preview of the verification network status</p></div><NetworkVisual dashboard /></StyledCard><div className="grid gap-5 md:grid-cols-3"><Stat label="Reputation score" value="—" note="Connect wallet to view" /><Stat label="Credentials" value="—" note="No credentials issued" /><Stat label="Network activity" value="—" note="Awaiting first transaction" /></div><div className="mt-5 grid gap-5 lg:grid-cols-[1.4fr_1fr]"><StyledCard><div className="flex items-center justify-between border-b border-primary/20 pb-4"><h2 className="font-semibold">Signal overview</h2><BarChart3 className="size-4 text-muted-foreground" /></div><div className="mt-6 space-y-4"><div className="flex items-center justify-between rounded-lg border border-border bg-background/50 p-4"><div><p className="text-sm font-medium">Verified signals</p><p className="mt-1 text-xs text-muted-foreground">Total verification events</p></div><span className="text-2xl font-bold text-primary">—</span></div><div className="flex items-center justify-between rounded-lg border border-border bg-background/50 p-4"><div><p className="text-sm font-medium">Trust connections</p><p className="mt-1 text-xs text-muted-foreground">Linked identities</p></div><span className="text-2xl font-bold text-primary">—</span></div><div className="flex items-center justify-between rounded-lg border border-border bg-background/50 p-4"><div><p className="text-sm font-medium">Attestations</p><p className="mt-1 text-xs text-muted-foreground">Third-party verifications</p></div><span className="text-2xl font-bold text-primary">—</span></div></div><p className="mt-4 text-xs text-muted-foreground italic">Connect a wallet to enable signal analysis</p></StyledCard><StyledCard><div className="border-b border-primary/20 pb-4"><h2 className="font-semibold">Connected identity</h2><p className="mt-1 text-xs text-muted-foreground">Link your wallet to view reputation data</p></div><div className="mt-5 flex flex-col gap-3"><div className="flex items-center justify-between border-b border-border pb-3 text-sm"><span className="text-muted-foreground">Wallet</span><span className="text-amber-400">Not connected</span></div><div className="flex items-center justify-between border-b border-border pb-3 text-sm"><span className="text-muted-foreground">Network</span><span className="text-primary">Solana Devnet</span></div><div className="flex items-center justify-between border-b border-border pb-3 text-sm"><span className="text-muted-foreground">Status</span><span className="flex items-center gap-2"><span className="size-2 rounded-full bg-primary shadow-[0_0_8px_#22d3ee] animate-pulse" />Active</span></div><div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">Privacy mode</span><span className="text-green-400">Enabled</span></div><div className="mt-4"><WalletControl /></div></div></StyledCard></div><div className="mt-5"><StyledCard><div className="flex items-center justify-between border-b border-primary/20 pb-4"><h2 className="font-semibold">Recent activity</h2><Activity className="size-4 text-muted-foreground" /></div><Empty icon={Activity} title="No activity recorded" description="Your verification events and credential issuance will appear here once you connect your wallet." /></StyledCard></div></> : sub === "agents" ? <div className="flex flex-col gap-5"><StyledCard><div className="flex items-center justify-between"><div><h2 className="font-semibold">Registered agents</h2><p className="mt-1 text-sm text-muted-foreground">Agents will use scoped credentials to act on your behalf.</p></div><Button variant="outline">Register agent <ArrowRight className="size-4" /></Button></div></StyledCard><Empty icon={Sparkles} title="No agents registered" description="Create an agent when you are ready to automate a trusted workflow." /></div> : sub === "settings" ? <StyledCard><h2 className="font-semibold">Workspace settings</h2><div className="mt-6 flex flex-col gap-4"><label className="flex items-center justify-between rounded-lg border border-border p-4 text-sm"><span><span className="block font-medium">Privacy mode</span><span className="text-xs text-muted-foreground">Only share credentials when explicitly requested.</span></span><input type="checkbox" defaultChecked className="accent-primary" /></label><label className="flex items-center justify-between rounded-lg border border-border p-4 text-sm"><span><span className="block font-medium">Activity notifications</span><span className="text-xs text-muted-foreground">Receive updates for new attestations.</span></span><input type="checkbox" className="accent-primary" /></label></div></StyledCard> : <><div className="grid gap-5 md:grid-cols-3"><Stat label="Verified signals" /><Stat label="Trust connections" /><Stat label="Credentials" /></div><div className="mt-5"><Empty icon={sub === "activity" ? Activity : sub === "credentials" ? ShieldCheck : sub === "analysis" ? BarChart3 : UserRound} title={sub ? `No ${sub} data yet` : "Connect your wallet to begin"} description="This workspace is connected to the CredLayer protocol, but no live records are available yet." /></div></>}</div></Shell>; }
